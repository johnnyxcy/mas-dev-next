...
class FunctionSignature(object):
    ...
    def n_fatal_errors(cls):
        return (0 if cls.ignore_invalid_defaultarg else
                cls.n_invalid_default_values) + (0 if cls.ignore_invalid_signature else cls.n_invalid_signatures)
...
class StubsGenerator(object):
    ...
    GLOBAL_CLASSNAME_REPLACEMENTS = {
        re.compile(r"numpy.ndarray\[(?P<type>[^\[\]]+)(\[(?P<shape>[^\[\]]+)\])?(?P<extra>[^][]*)\]"):
            replace_numpy_array,
        re.compile(
...
class AttributeStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        if len(value_lines) == 1:
            ...
            return (
                ["{name}: {typename} # value = ".format(name=self.name, typename=typename)] + ['"""'] +
...
class FreeFunctionStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        for sig in self.signatures:
            ...
            result.append("def {name}({args}) -> {rtype}:".format(name=sig.name, args=sig.args, rtype=sig.rtype))
            if docstring:
...
class ClassStubsGenerator(StubsGenerator):
    ...
    def parse(self):
        ...
        for name, member in inspect.getmembers(self.klass):
            ...
            if inspect.isroutine(member):
                ...
                name in self.attributes_blacklist or (is_pybind11 and name in self.pybind11_attributes_blacklist)
            ):
                self.fields.append(AttributeStubsGenerator(name, member))
                # logger.warning("Unknown member %s type : `%s` " % (name, str(type(member))))

        for x in itertools.chain(self.classes, self.methods, self.properties, self.fields):
            x.parse()
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        base_classes_list = [
            strip_current_module_name(self.fully_qualified_name(b), self.klass.__module__) for b in self.base_classes
        ]
        result = [
...
class ModuleStubsGenerator(StubsGenerator):
    ...
    def to_lines(self):  # type: () -> List[str]
        ...
        for name, class_ in self.imported_classes.items():
            ...
            result += ["from {} import {}{}".format(class_.__module__, class_name, suffix)]
        ...
        if self.submodules:
            result.extend(
                map(lambda mod: "from . import {}".format(mod.module.__name__.split(".")[-1]), self.submodules)
            )

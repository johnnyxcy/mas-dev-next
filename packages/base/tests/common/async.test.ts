/*
 * File: @mas/base/tests/common/async.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 09/27/2023 03:47 pm
 *
 * Last Modified: 09/27/2023 04:05 pm
 *
...
describe("Async", () => {
    ...
    describe("cancelablePromise", () => {
        ...
        test("execution order (sync)", async () => {
            const order: string[] = [];
        ...
        test("execution order (async)", async () => {
            const order: string[] = [];

            const cancellablePromise = async.createCancelablePromise((token) => {
                order.push("in callback");
                store.add(token.onCancellationRequested((_) => order.push("cancelled")));
                return new Promise((resolve) => setTimeout(resolve.bind(1234), 0));
            });
    ...
    describe("Delayer", () => {
        ...
        describe("ThrottledDelayer", () => {
            ...
            test("trigger after dispose throws", async () => {
                const throttledDelayer = new async.ThrottledDelayer<void>(100);
                throttledDelayer.dispose();
                await expect(() => throttledDelayer.trigger(async () => {}, 0)).rejects.toThrow(
        ...
        test("cancel should cancel all calls to trigger", async () => {
            let count = 0;
            const factory = () => {
                return Promise.resolve(++count);
            };
    ...
    describe("Limiter", () => {
        test("assert degree of paralellism", async () => {
    ...
    describe("Queue", () => {
        ...
        test("errors bubble individually but not cause stop", async () => {
            const queue = new async.Queue();
        ...
        test("order is kept (chained)", async () => {
            const queue = new async.Queue();
            ...
            return queue.queue(f1).then(async () => {

/*
 * File: @mas/base/tests/common/glob.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 09/27/2023 06:22 pm
 *
 * Last Modified: 09/27/2023 06:26 pm
 *
...
import * as glob from "@mas/base/common/glob";
...
describe("Glob", () => {
    ...
    test("expression support (single)", function () {
        ...
        const hasSibling = (name: string) => siblings.includes(name);
    ...
    test("expression with other falsy value", function () {
        const expr = { "**/*.js": 0 } as any as { [key: string]: boolean };
    ...
    function testOptimizationForBasenames(
        ...
        matches.forEach(([text, result], i) => {
            assert.strictEqual(parsed(text, null!, siblingsFns[i]), result as any);
        });
    }
    ...
    test("expression/pattern path", function () {
        ...
        assert.strictEqual(
            glob.parse("**/foo/bar/**", { trimForExclusions: true })(nativeSep("bar/foo/bar"), "bar"),
            true,
        );
        assert.strictEqual(
        ...
        assert.strictEqual(
            glob.parse("foo/bar/**", { trimForExclusions: true })(nativeSep("foo/bar/baz"), "baz"),
            false,
        );
    });
    ...
    test("expression/pattern optimization for paths", function () {
        ...
        assert.deepStrictEqual(glob.getPathTerms(glob.parse("**/foo/bar/**", { trimForExclusions: true })), [
            "*/foo/bar",
        ]);
    ...
    test("expression fails when siblings use promises (https://github.com/microsoft/vscode/issues/146294)", async function () {
        ...
        const hasSibling = (name: string) => Promise.resolve(siblings.includes(name));
    ...
    test("patternsEquals", () => {
        ...
        assert.ok(
            glob.patternsEquals([{ base: "a", pattern: "*" }, "b", "c"], [{ base: "a", pattern: "*" }, "b", "c"]),
        );

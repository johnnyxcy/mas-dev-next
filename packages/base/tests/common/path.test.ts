 ...
 *
 * File Created: 09/27/2023 11:22 am
 *
 * Last Modified: 09/27/2023 11:26 am
 *
...
import { isWeb, isWindows } from "@mas/base/common/platform";
...
describe("Paths (Node Implementation)", () => {
    ...
    test("join", () => {
        ...
        joinTests.forEach((testArr: any[]) => {
            if (!Array.isArray(testArr[0])) {
                testArr[0] = [testArr[0]];
            }
            testArr[0].forEach((join: any) => {
                testArr[1].forEach((test: any) => {
    ...
    test("resolve", () => {
        ...
        resolveTests.forEach((testArr) => {
            const resolve = testArr[0];
            // @ts-expect-error should exists
            testArr[1].forEach((test) => {
                // @ts-expect-error should exists
                ...
                const expected = test[1];
                const message = `path.${os}.resolve(${test[0]
                    .map(JSON.stringify)
                    .join(",")})\n  expect=${JSON.stringify(expected)}\n  actual=${JSON.stringify(actual)}`;
                if (actual !== expected && actualAlt !== expected) {
                    failures.push(`\n${message}`);
    ...
    test("relative", () => {
        ...
        relativeTests.forEach((testArr) => {
            const relative = testArr[0];
            // @ts-expect-error should exists
            testArr[1].forEach((test) => {
                // @ts-expect-error should exists

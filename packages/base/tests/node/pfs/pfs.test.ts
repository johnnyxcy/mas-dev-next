/*
 * File: @mas/base/tests/node/pfs/pfs.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/12/2023 01:13 pm
 *
 * Last Modified: 10/12/2023 01:18 pm
 *
...
import { basename, dirname, join, resolve, sep } from "@mas/base/common/path";
...
describe("PFS", function () {
    ...
    test("readDirsInDir", async () => {
        ...
        assert.ok(result.includes("somefolder1"));
        assert.ok(result.includes("somefolder2"));
        assert.ok(result.includes("somefolder3"));
    });
    ...
    test("readdir", async () => {
        if (typeof process.versions["electron"] !== "undefined" /* needs electron */) {
            ...
            assert.strictEqual(children.includes("öäü"), true); // Mac always converts to NFD, so
        }
    });
    ...
    test("writeFile (UInt8Array)", async () => {
        ...
        return testWriteFile(
            BinaryBuffer.fromString(smallData).buffer,
            smallData,
            BinaryBuffer.fromString(bigData).buffer,
            bigData,
        );
    });

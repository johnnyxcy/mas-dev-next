...
import { BrowserFeatures } from "@mas/base/browser/can-i-use";
import { IKeyboardEvent, StandardKeyboardEvent } from "@mas/base/browser/keyboard-event";
import { IMouseEvent, StandardMouseEvent } from "@mas/base/browser/mouse-event";
import { MaspectraWindow, ensureMaspectraWindow, mainWindow } from "@mas/base/browser/window";
import { AbstractIdleValue, IdleDeadline, IntervalTimer, TimeoutTimer, _runWhenIdle } from "@mas/base/common/async";
...
import { KeyCode } from "@mas/base/common/key-codes";
import { Disposable, DisposableStore, IDisposable, toDisposable } from "@mas/base/common/lifecycle";
import * as platform from "@mas/base/common/platform";
import { URI } from "@mas/base/common/uri";

export interface IRegisteredMaspectraWindow {
    readonly window: MaspectraWindow;
...
export const {
    ...
    const windows = new Map<number, IRegisteredMaspectraWindow>();

    ensureMaspectraWindow(mainWindow, 1);
    windows.set(mainWindow.vscodeWindowId, { window: mainWindow, disposables: new DisposableStore() });

    const onDidRegisterWindow = new event.Emitter<IRegisteredMaspectraWindow>();
    const onDidUnregisterWindow = new event.Emitter<MaspectraWindow>();
    const onWillUnregisterWindow = new event.Emitter<MaspectraWindow>();

    return {
        onDidRegisterWindow: onDidRegisterWindow.event,
        onWillUnregisterWindow: onWillUnregisterWindow.event,
        onDidUnregisterWindow: onDidUnregisterWindow.event,
        registerWindow(window: MaspectraWindow): IDisposable {
        ...
        getWindows(): Iterable<IRegisteredMaspectraWindow> {
        ...
        getWindowId(targetWindow: Window): number {
            return (targetWindow as MaspectraWindow).vscodeWindowId;
        },
        hasWindow(windowId: number): boolean {
            return windows.has(windowId);
        },
        getWindowById(windowId: number): IRegisteredMaspectraWindow | undefined {
            return windows.get(windowId);
        },
        getWindow(e: Node | UIEvent | undefined | null): MaspectraWindow {
            const candidateNode = e as Node | undefined | null;
            if (candidateNode?.ownerDocument?.defaultView) {
                return candidateNode.ownerDocument.defaultView.window as MaspectraWindow;
            }

            const candidateEvent = e as UIEvent | undefined | null;
            if (candidateEvent?.view) {
                return candidateEvent.view.window as MaspectraWindow;
            }
...
export function getActiveWindow(): MaspectraWindow {
    const document = getActiveDocument();
    return (document.defaultView?.window ?? mainWindow) as MaspectraWindow;
}
...
export class ModifierKeyEmitter extends event.Emitter<IModifierKeyStatus> {
    ...
    get isModifierPressed(): boolean {
        return (
            this._keyStatus.altKey || this._keyStatus.ctrlKey || this._keyStatus.metaKey || this._keyStatus.shiftKey
        );
    }

/*
 * File: @mas/base/src/testing/common/time-travel-scheduler.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 09/27/2023 04:00 pm
 *
 * Last Modified: 09/27/2023 04:00 pm
 *
...
import { setTimeout0, setTimeout0IsFaster } from "@mas/base/common/platform";
...
export class TimeTravelScheduler implements Scheduler {
    ...
    readonly onTaskScheduled = this.taskScheduledEmitter.event;
...
export class AsyncSchedulerProcessor extends Disposable {
    ...
    readonly onTaskQueueEmpty = this.queueEmptyEmitter.event;
    ...
    private process() {
        ...
        if (executedTask) {
            ...
            if (this.history.length >= this.maxTaskCount && this.scheduler.hasScheduledTasks) {
                ...
                const e = new Error(
                    `Queue did not get empty after processing ${this.history.length} items. These are the last ${
                        lastTasks.length
                    } scheduled tasks:\n${lastTasks.join("\n\n\n")}`,
                );
                this.lastError = e;

/*
 * File: @mas/base/src/common/encoding/index.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/11/2023 11:06 am
 *
...
import { Readable, ReadableStream, listenStream, newWriteableStream } from "@mas/base/common/stream";
...
export function isUTFEncoding(encoding: string): encoding is UTF_ENCODING {
    return [UTF8, UTF8_with_bom, UTF16be, UTF16le].includes(encoding);
}
...
export function toDecodeStream(
    source: BinaryBufferReadableStream,
        ...
        const createDecoder = async () => {
            try {
                ...
                const detected = await detectEncodingFromBuffer(
                    {
                        buffer: BinaryBuffer.concat(bufferedChunks),
                ...
                const decoded = decoder.write(BinaryBuffer.concat(bufferedChunks).buffer);

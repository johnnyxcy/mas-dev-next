/*
 * File: @mas/core/tests/platform/files/common/watcher.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/02/2023 03:22 pm
 *
 * Last Modified: 11/02/2023 03:33 pm
 *
...
import { FileChangeType, FileChangesEvent, IFileChange } from "@mas/core/platform/files/common/files";
import { coalesceEvents, parseWatcherPatterns, reviveFileChanges } from "@mas/core/platform/files/common/watcher";
...
describe("Watcher", () => {
    test("parseWatcherPatterns - posix", (ctx) => {
        if (isWindows) {
            ctx.skip();
        }
    ...
    test("parseWatcherPatterns - windows", (ctx) => {
        if (!isWindows) {
            ctx.skip();
        }
...
describe("Watcher Events Normalizer", () => {
    ...
    (isWindows ? [Path.WINDOWS, Path.UNC] : [Path.UNIX]).forEach((path) => {
        test(`delete only reported for top level folder (${path})`, () =>
            new Promise<void>((done) => {
                const watch = disposables.add(new TestFileWatcher());
    ...
    test("event coalescer: ignore CREATE followed by DELETE", () =>
        new Promise<void>((done) => {
            const watch = disposables.add(new TestFileWatcher());
    ...
    test("event coalescer: flatten DELETE followed by CREATE into CHANGE", () =>
        new Promise<void>((done) => {
            const watch = disposables.add(new TestFileWatcher());
    ...
    test("event coalescer: ignore UPDATE when CREATE received", () =>
        new Promise<void>((done) => {
            const watch = disposables.add(new TestFileWatcher());
    ...
    test("event coalescer: apply DELETE", () =>
        new Promise<void>((done) => {
            const watch = disposables.add(new TestFileWatcher());
    ...
    test("event coalescer: track case renames", () =>
        new Promise<void>((done) => {
            const watch = disposables.add(new TestFileWatcher());

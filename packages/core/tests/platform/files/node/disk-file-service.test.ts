/*
 * File: @mas/core/tests/platform/files/node/disk-file-service.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/06/2023 05:02 pm
 *
 * Last Modified: 11/06/2023 05:12 pm
 *
...
import { Schemas } from "@mas/base/common/network";
import { basename, dirname, join, posix } from "@mas/base/common/path";
import { isLinux, isWindows } from "@mas/base/common/platform";
import { joinPath } from "@mas/base/common/resources";
...
} from "@mas/core/platform/files/common/files";
import { DiskFileSystemProvider } from "@mas/core/platform/files/node/disk-file-system-provider";
import { NullLogService } from "@mas/core/platform/log/common/log";
...
function toLineByLineReadable(content: string): BinaryBufferReadable {
    ...
    return {
        read(): BinaryBuffer | null {
            ...
            if (typeof chunk === "string") {
                return BinaryBuffer.fromString(chunk);
            }
...
describe(
    ...
    function () {
        ...
        setup(async () => {
            ...
            const sourceDir = join(__dirname, "fixtures/service");
        ...
        test("resolve - file", async () => {
            const resource = joinPath(URI.file(__dirname), "fixtures/resolver/index.html");
        ...
        test("resolve - directory", async () => {
            ...
            const resource = joinPath(URI.file(__dirname), "fixtures/resolver");
            ...
            assert.ok(
                result.children!.every((entry) => {
                    return testsElements.includes(basename(entry.resource.fsPath));
                }),
            );

            result.children!.forEach((value) => {
                assert.ok(basename(value.resource.fsPath));
                if (["examples", "other"].includes(basename(value.resource.fsPath))) {
        ...
        test("resolve - directory - resolveTo single directory", async () => {
            const resolverFixturesPath = join(__dirname, "fixtures/resolver");
        ...
        async function testResolveDirectoryWithTarget(withQueryParam: boolean): Promise<void> {
            ...
            const result = await service.resolve(
                URI.file(resolverFixturesPath).with({ query: withQueryParam ? "test" : undefined }),
                {
                    resolveTo: [
        ...
        test("resolve directory - resolveSingleChildFolders", async () => {
            const resolverFixturesPath = join(__dirname, "fixtures/resolver/other");
        ...
        test("resolve - file symbolic link", async (ctx) => {
            /* windows: cannot create file symbolic link without elevated context */
            if (isWindows) {
                ctx.skip();
            }
        ...
        test("deleteFile (useTrash)", async (ctx) => {
            /* trash is unreliable on Linux */
            if (isLinux) {
                ctx.skip();
            }
        ...
        test("deleteFile - symbolic link (exists)", async (ctx) => {
            /* windows: cannot create file symbolic link without elevated context */
            if (isWindows) {
                ctx.skip();
            }
        ...
        test("deleteFile - symbolic link (pointing to nonexistent file)", async (ctx) => {
            /* windows: cannot create file symbolic link without elevated context */
            if (isWindows) {
                ctx.skip();
            }
        ...
        test("deleteFolder (recursive, useTrash)", async (ctx) => {
            /* trash is unreliable on Linux */
            if (isLinux) {
                ctx.skip();
            }
        ...
        async function testDeleteFolderRecursive(
            useTrash: boolean,
        ...
        test("move - multi folder", async () => {
            ...
            assert.strictEqual(
                await service.canMove(source, URI.file(join(dirname(source.fsPath), renameToPath))),
                true,
            );
        ...
        test("move - source parent of target", async () => {
            ...
            assert.ok(
                (await service.canMove(URI.file(testDir), URI.file(join(testDir, "binary.txt")))) instanceof Error,
            );
        ...
        test("move - FILE_MOVE_CONFLICT", async () => {
            ...
            assert.ok(
                (await service.canMove(source.resource, URI.file(join(testDir, "binary.txt")))) instanceof Error,
            );
        ...
        function setCapabilities(
            provider: TestDiskFileSystemProvider,
        ...
        test("copy - MIX CASE same target - no overwrite", async () => {
            ...
            if (isLinux) {
                ...
                assert.strictEqual(existsSync(copied!.resource.fsPath), true);
                assert.ok(readdirSync(testDir).includes("INDEX.html"));
        ...
        test("copy - MIX CASE different target - overwrite", async () => {
            ...
            assert.strictEqual(existsSync(renamed.resource.fsPath), true);
            assert.ok(readdirSync(testDir).includes("CONWAY.js"));
        ...
        test("readFile - FILE_NOT_DIRECTORY", async (ctx) => {
            /* error code does not seem to be supported on windows */
            if (isWindows) {
                ctx.skip();
            }
        ...
        async function doTestFileTooLarge(statSizeWrong: boolean) {
            ...
            if (!statSizeWrong) {
                assert(error instanceof TooLargeFileOperationError);
                assert.ok(typeof error.size === "number");
            }
            assert.strictEqual(error!.fileOperationResult, FileOperationResult.FILE_TOO_LARGE);
        }

        test("readFile - dangling symbolic link - https://github.com/microsoft/vscode/issues/116049", async (ctx) => {
            /* windows: cannot create file symbolic link without elevated context */
            if (isWindows) {
                ctx.skip();
            }
        ...
        test("createFile", async () => {
            return assertCreateFile((contents) => BinaryBuffer.fromString(contents));
        });

        test("createFile (readable)", async () => {
            return assertCreateFile((contents) => bufferToReadable(BinaryBuffer.fromString(contents)));
        });

        test("createFile (stream)", async () => {
            return assertCreateFile((contents) => bufferToStream(BinaryBuffer.fromString(contents)));
        });

        async function assertCreateFile(
            converter: (content: string) => BinaryBuffer | BinaryBufferReadable | BinaryBufferReadableStream,
        ): Promise<void> {
            let event: FileOperationEvent;
            disposables.add(service.onDidRunOperation((e) => (event = e)));
        ...
        test("createFile (does not overwrite by default)", async () => {
            ...
            let error;
            try {
                await service.createFile(resource, BinaryBuffer.fromString(contents));
            } catch (err) {
                error = err;
            }
        ...
        test("createFile (allows to overwrite existing)", async () => {
            ...
            const fileStat = await service.createFile(resource, BinaryBuffer.fromString(contents), {
                overwrite: true,
            });
            assert.strictEqual(fileStat.name, "test.txt");
            assert.strictEqual(existsSync(fileStat.resource.fsPath), true);
        ...
        test("writeFile - atomic writing does not break symlinks", async (ctx) => {
            /* windows: cannot create file symbolic link without elevated context */
            if (isWindows) {
                ctx.skip();
            }
            ...
            await service.writeFile(link, BinaryBuffer.fromString(content), { atomic: { postfix: ".vsctmp" } });
        ...
        async function testWriteFile(atomic: boolean) {
            ...
            await service.writeFile(resource, BinaryBuffer.fromString(newContent), {
        ...
        async function testWriteFileLarge(atomic: boolean) {
            ...
            const fileStat = await service.writeFile(resource, BinaryBuffer.fromString(newContent), {
                atomic: atomic ? { postfix: ".vsctmp" } : false,
            });
        ...
        test("writeFile (large file) - unbuffered (atomic) - concurrent writes with multiple services", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                ...
                promises.push(
                    service.writeFile(resource, BinaryBuffer.fromString(`${newContent}${++suffix}`), {
        ...
        test("writeFile (large file) - multiple parallel writes queue up and atomic read support (via file service)", async () => {
            ...
            const writePromises = Promise.all(
                ["0", "00", "000", "0000", "00000"].map(async (offset) => {
                    const fileStat = await service.writeFile(resource, BinaryBuffer.fromString(offset + newContent));
                    assert.strictEqual(fileStat.name, "lorem.txt");
                }),
            );
        ...
        test("provider - write barrier prevents dirty writes", async () => {
            ...
            assert(provider);
            assert(hasOpenReadWriteCloseCapability(provider));

            const writePromises = Promise.all(
                ["0", "00", "000", "0000", "00000"].map(async (offset) => {
                    const content = offset + newContent;
                    const contentBuffer = BinaryBuffer.fromString(content).buffer;
                    ...
                    try {
                        await provider.write(
                            fd,
                            0,
                            BinaryBuffer.fromString(content).buffer,
                            0,
        ...
        test("provider - write barrier is partitioned per resource", async () => {
            ...
            try {
                await provider.write(
                    fd1,
                    0,
                    BinaryBuffer.fromString(newContent).buffer,
                    0,
                    BinaryBuffer.fromString(newContent).buffer.byteLength,
                );
        ...
        test("provider - write barrier not becoming stale", async () => {
            ...
            const newContent = content.toString() + content.toString();
            const newContentBuffer = BinaryBuffer.fromString(newContent).buffer;
        ...
        test("provider - atomic reads (write pending when read starts)", async () => {
            ...
            assert(provider);
            assert(hasOpenReadWriteCloseCapability(provider));
            assert(hasFileAtomicReadCapability(provider));
        ...
        test("writeFile (file is created including parents)", async () => {
            ...
            const fileStat = await service.writeFile(resource, BinaryBuffer.fromString(content));
        ...
        async function testLockedFiles(expectError: boolean) {
            ...
            const content = await service.writeFile(lockedFile, BinaryBuffer.fromString("Locked File"));
            ...
            try {
                await service.writeFile(lockedFile, BinaryBuffer.fromString(newContent));
            } catch (e) {
                error = e;
            }
            ...
            if (expectError) {
                try {
                    await service.writeFile(lockedFile, BinaryBuffer.fromString(newContent), { unlock: true });
                } catch (e) {
                    error = e;
                }
        ...
        test("writeFile (error when folder is encountered)", async () => {
            ...
            let error: Error | undefined = undefined;
            try {
                await service.writeFile(resource, BinaryBuffer.fromString("File is created including parent"));
            } catch (err) {
                error = err;
            }
        ...
        test("writeFile - error when writing to file that has been updated meanwhile", async () => {
            ...
            let error: FileOperationError | undefined = undefined;
            try {
                await service.writeFile(resource, BinaryBuffer.fromString(newContentLeadingToError), {
        ...
        test("writeFile - no error when writing to file where size is the same", async () => {
            ...
            let error: FileOperationError | undefined = undefined;
            try {
                await service.writeFile(resource, BinaryBuffer.fromString(newContentLeadingToNoError), {
        ...
        test("writeFile - no error when writing to same nonexistent folder multiple times different new files", async () => {
            ...
            const newContent = "Updates to the small file";
            await Promise.all([
                service.writeFile(file1, BinaryBuffer.fromString(newContent)),
                service.writeFile(file2, BinaryBuffer.fromString(newContent)),
                service.writeFile(file3, BinaryBuffer.fromString(newContent)),
            ]);
        ...
        test("writeFile - error when writing to folder that is a file", async () => {
            ...
            const newContent = "Updates to the small file";
            try {
                await service.writeFile(newFile, BinaryBuffer.fromString(newContent));
            } catch (e) {
                error = e;
            }
        ...
        test("read - mixed positions", async () => {
            ...
            let buffer = BinaryBuffer.alloc(1024);
            ...
            buffer = BinaryBuffer.alloc(1024);
        ...
        test("write - mixed positions", async () => {
            ...
            const buffer = BinaryBuffer.alloc(1024);
            ...
            const initialContents = BinaryBuffer.fromString("Lorem ipsum dolor sit amet");
            ...
            const contents = BinaryBuffer.fromString("Hello World");
        ...
        test("readonly - is handled properly for a single resource", async () => {
            ...
            try {
                await service.writeFile(resource, BinaryBuffer.fromString("Hello Test"));
            } catch (error) {

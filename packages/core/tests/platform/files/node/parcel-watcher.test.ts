/*
 * File: @mas/core/tests/platform/files/node/parcel-watcher.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/06/2023 04:14 pm
 *
 * Last Modified: 11/06/2023 05:00 pm
 *
...
import { dirname, join } from "@mas/base/common/path";
import { isLinux, isMacintosh, isWindows } from "@mas/base/common/platform";
import { extUriBiasedIgnorePathCase } from "@mas/base/common/resources";
import { URI } from "@mas/base/common/uri";
import { Promises, RimRafMode } from "@mas/base/node/pfs";
...
import { IRecursiveWatchRequest } from "@mas/core/platform/files/common/watcher";
    ...
    () => {
        class TestParcelWatcher extends ParcelWatcher {
            testNormalizePaths(paths: string[], excludes: string[] = []): string[] {
                ...
                return this.normalizeRequests(requests, false /* validate paths skipped for tests */).map(
        ...
        async function awaitEvent(
            watcher: TestParcelWatcher,
            path: string,
            type: FileChangeType,
            failOnEventReason?: string,
            ...
            const res = await new Promise<IFileChange[]>((resolve, reject) => {
                ...
                const disposable = watcher.onDidChangeFile((events) => {
                    for (const event of events) {
                        if (
                            extUriBiasedIgnorePathCase.isEqual(event.resource, URI.file(path)) &&
                            event.type === type &&
        ...
        /* this test seems not possible with fsevents backend */
        test("basics (atomic writes)", async function (ctx) {
            if (isMacintosh) {
                ctx.skip();
            }
            await watcher.watch([{ path: testDir, excludes: [], recursive: true }]);
        ...
        /* polling is only used in linux environments (WSL) */
        test("basics (polling)", async function (ctx) {
            if (!isLinux) {
                ctx.skip();
            }
        ...
        test("subsequent watch updates watchers (path)", async function () {
            ...
            await watcher.watch([
                { path: join(testDir, "deep"), excludes: [join(realpathSync(testDir), "unrelated")], recursive: true },
            ]);
        ...
        test("symlink support (root)", async function (ctx) {
            if (isWindows) {
                ctx.skip();
            }
        ...
        /* windows: cannot create file symbolic link without elevated context */
        test("symlink support (via extra watch)", async function (ctx) {
            if (isWindows) {
                ctx.skip();
            }
            const link = join(testDir, "deep-linked");
        ...
        test("wrong casing", async function (ctx) {
            if (isLinux) {
                ctx.skip();
            }
        ...
        /* flaky on windows */
        test("deleting watched path is handled properly", async function (ctx) {
            if (isWindows) {
                ctx.skip();
            }
        ...
        test("should not exclude roots that do not overlap", () => {
            if (isWindows) {
                ...
                assert.deepStrictEqual(watcher.testNormalizePaths(["C:\\a", "C:\\b", "C:\\c\\d\\e"]), [
                    "C:\\a",
                    "C:\\b",
                    "C:\\c\\d\\e",
                ]);
        ...
        test("should remove sub-folders of other paths", () => {
            if (isWindows) {
                ...
                assert.deepStrictEqual(watcher.testNormalizePaths(["C:\\b\\a", "C:\\a", "C:\\b", "C:\\a\\b"]), [
                    "C:\\a",
                    "C:\\b",
                ]);

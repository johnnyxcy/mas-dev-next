/*
 * File: @mas/core/tests/platform/files/browser/file-services.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/02/2023 04:21 pm
 *
 * Last Modified: 11/02/2023 04:23 pm
 *
...
import { DeferredPromise, timeout } from "@mas/base/common/async";
import { BinaryBuffer, bufferToReadable, bufferToStream } from "@mas/base/common/buffer";
...
import { ReadableStreamEvents, consumeStream, newWriteableStream } from "@mas/base/common/stream";
import { URI } from "@mas/base/common/uri";
import { ensureNoDisposablesAreLeakedInTestSuite, teardown } from "@mas/base/testing/common/utils";
...
import { NullFileSystemProvider } from "@mas/core/platform/files/common/null-file-system-provider";
...
describe("File Service", () => {
    ...
    test("watch - with corelation", async () => {
        ...
        const watcher0 = disposables.add(
            service.watch(URI.parse("test://watch/folder1"), { recursive: true, excludes: [], includes: [] }),
        );
    ...
    test("enforced atomic read/write/delete", async () => {
        ...
        await service.writeFile(atomicResource, BinaryBuffer.fromString(""));
        await service.writeFile(nonAtomicResource, BinaryBuffer.fromString(""));

        await service.writeFile(atomicResource, bufferToStream(BinaryBuffer.fromString("")));
        await service.writeFile(nonAtomicResource, bufferToStream(BinaryBuffer.fromString("")));

        await service.writeFile(atomicResource, bufferToReadable(BinaryBuffer.fromString("")));
        await service.writeFile(nonAtomicResource, bufferToReadable(BinaryBuffer.fromString("")));

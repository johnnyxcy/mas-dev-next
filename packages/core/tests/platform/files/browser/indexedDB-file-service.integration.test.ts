/*
 * File: @mas/core/tests/platform/files/browser/indexedDB-file-service.integration.test.ts
 *
...
import { IndexedDB } from "@mas/base/browser/indexedDB";
import {
    BinaryBuffer,
...
describe(
    ...
    function () {
        ...
        const initFixtures = async () => {
            ...
            await Promise.all(
                (
                    ...
                    .map(([uri, contents]) => service.createFile(uri, BinaryBuffer.fromString(contents))),
            );
        };
        ...
        test("resolve - directory", async () => {
            ...
            assert.ok(
                result.children!.every((entry) => {
                    return testsElements.includes(basename(entry.resource));
                }),
            );

            result.children!.forEach((value) => {
                assert.ok(basename(value.resource));
                if (["examples", "other"].includes(basename(value.resource))) {
        ...
        const fileCreateBatchTester = (size: number, name: string) => {
            ...
            return {
                    ...
                    return (creationPromises = Promise.all(
                        batch.map((entry) =>
                            userdataFileProvider.writeFile(
                                entry.resource,
                                BinaryBuffer.fromString(entry.contents).buffer,
                                {
                                    create: true,
                                    overwrite: true,
                                    unlock: false,
                ...
                async assertContentsCorrect() {
                    if (!creationPromises) {
                        throw new Error("read called before create");
                    }
        ...
        test("rename to an existing file without overwrite", async () => {
            const parent = await service.resolve(userdataURIFromPaths([]));
            const sourceFile = joinPath(parent.resource, "sourceFile");
            await service.writeFile(sourceFile, BinaryBuffer.fromString("This is source file"));

            const targetFile = joinPath(parent.resource, "targetFile");
            await service.writeFile(targetFile, BinaryBuffer.fromString("This is target file"));
        ...
        test("rename to an existing file with overwrite", async () => {
            ...
            await Promise.all([
                service.writeFile(sourceFile, BinaryBuffer.fromString("This is source file")),
                service.writeFile(targetFile, BinaryBuffer.fromString("This is target file")),
            ]);
        ...
        test("rename a folder that has multiple files and folders", async () => {
            ...
            await Promise.all([
                service.writeFile(sourceFile1, BinaryBuffer.fromString("Source File 1")),
                service.writeFile(sourceFile2, BinaryBuffer.fromString("Source File 2")),
        ...
        test("rename a folder to another folder that has some files", async () => {
            ...
            await Promise.all([
                service.writeFile(sourceFile1, BinaryBuffer.fromString("Source File 1")),
                service.writeFile(targetFile2, BinaryBuffer.fromString("Target File 2")),
                service.writeFile(targetFile3, BinaryBuffer.fromString("Target File 3")),
            ]);

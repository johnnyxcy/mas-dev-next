/*
 * File: @mas/core/tests/parts/ipc/tests/node/ipc.net.test.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/24/2023 02:51 pm
 *
 * Last Modified: 10/24/2023 03:52 pm
 *
...
import { AddressInfo, Server, Socket, connect, createServer } from "node:net";
import { tmpdir } from "node:os";
import { assert, describe, test } from "vitest";

import { Barrier, timeout } from "@mas/base/common/async";
import { BinaryBuffer } from "@mas/base/common/buffer";
import { Emitter, Event } from "@mas/base/common/event";
import { Disposable, DisposableStore } from "@mas/base/common/lifecycle";
import { runWithFakedTimers } from "@mas/base/testing/common/time-travel-scheduler";
import { ensureNoDisposablesAreLeakedInTestSuite, setup } from "@mas/base/testing/common/utils";
import {
    ILoadEstimator,
    PersistentProtocol,
    Protocol,
...
} from "@mas/core/parts/ipc/node/ipc.net";

class MessageStream extends Disposable {
    private _currentComplete: ((data: BinaryBuffer) => void) | null;
    private _messages: BinaryBuffer[];
    ...
    waitForOne(): Promise<BinaryBuffer> {
        return new Promise<BinaryBuffer>((resolve) => {
            this._currentComplete = resolve;
            this._trigger();
        });
    }
}
...
class Ether {
    ...
    write(from: "a" | "b", data: Buffer): void {
        setTimeout(() => {
            if (from === "a") {
                this._ab.push(data);
            } else {
...
describe("IPC, Socket Protocol", () => {
    ...
    test("read/write, object data", async () => {
        ...
        a.send(BinaryBuffer.fromString(JSON.stringify(data)));
...
describe("PersistentProtocol reconnection", () => {
    ...
    test("acks get piggybacked with messages", async () => {
        ...
        a.send(BinaryBuffer.fromString("a1"));
        assert.strictEqual(a.unacknowledgedCount, 1);
        assert.strictEqual(b.unacknowledgedCount, 0);

        a.send(BinaryBuffer.fromString("a2"));
        assert.strictEqual(a.unacknowledgedCount, 2);
        assert.strictEqual(b.unacknowledgedCount, 0);

        a.send(BinaryBuffer.fromString("a3"));
        ...
        b.send(BinaryBuffer.fromString("b1"));
        assert.strictEqual(a.unacknowledgedCount, 3);
        ...
        a.send(BinaryBuffer.fromString("a4"));
        assert.strictEqual(a.unacknowledgedCount, 1);
        assert.strictEqual(b.unacknowledgedCount, 1);
...
describe("WebSocketNodeSocket", () => {
    ...
    class FakeNodeSocket extends Disposable {
        private readonly _onData = new Emitter<BinaryBuffer>();
        readonly onData = this._onData.event;

        private readonly _onClose = new Emitter<SocketCloseEvent>();
        readonly onClose = this._onClose.event;

        writtenData: BinaryBuffer[] = [];
        ...
        write(data: BinaryBuffer): void {
            this.writtenData.push(data);
        }

        fireData(data: number[]): void {
            this._onData.fire(BinaryBuffer.wrap(toUint8Array(data)));
        }
    }
    ...
    function generateRandomBuffer(size: number): BinaryBuffer {
        const buff = BinaryBuffer.alloc(size);

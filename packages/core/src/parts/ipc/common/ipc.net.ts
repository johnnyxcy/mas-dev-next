/*
 * File: @mas/core/src/parts/ipc/common/ipc.net.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/18/2023 03:16 pm
 *
...
import { Disposable, DisposableStore, IDisposable } from "@mas/base/common/lifecycle";
import { IIPCLogger, IMessagePassingProtocol, IPCClient } from "@mas/core/parts/ipc/common/ipc";
...
export namespace SocketDiagnostics {
    ...
    export function traceSocketEvent(
        ...
        if (
            data instanceof BinaryBuffer ||
            data instanceof Uint8Array ||
            data instanceof ArrayBuffer ||
            ArrayBuffer.isView(data)
        ) {
            const copiedData = BinaryBuffer.alloc(data.byteLength);
            copiedData.set(data);
            records.push({ timestamp: Date.now(), id, label: socketDebugLabel, type, buff: copiedData });
        } else {
            // data is a custom object
            records.push({ timestamp: Date.now(), id, label: socketDebugLabel, type, data });
        }
    }
}
...
let emptyBuffer: BinaryBuffer | null = null;
function getEmptyBuffer(): BinaryBuffer {
    if (!emptyBuffer) {
        emptyBuffer = BinaryBuffer.alloc(0);
    }
    return emptyBuffer;
}

export class ChunkStream {
    private _chunks: BinaryBuffer[];
    ...
    acceptChunk(buff: BinaryBuffer) {
        this._chunks.push(buff);
        this._totalLength += buff.byteLength;
    }

    read(byteCount: number): BinaryBuffer {
        return this._read(byteCount, true);
    }

    peek(byteCount: number): BinaryBuffer {
        return this._read(byteCount, false);
    }

    private _read(byteCount: number, advance: boolean): BinaryBuffer {
        if (byteCount === 0) {
        ...
        const result = BinaryBuffer.alloc(byteCount);
        let resultOffset = 0;
        let chunkIndex = 0;
        while (byteCount > 0) {
...
class ProtocolReader extends Disposable {
    ...
    readonly onMessage: Event<ProtocolMessage> = this._onMessage.event;
    ...
    acceptChunk(data: BinaryBuffer | null): void {
...
class ProtocolWriter {
    ...
    write(msg: ProtocolMessage) {
        ...
        const header = BinaryBuffer.alloc(ProtocolConstants.HeaderLength);
    ...
    private _bufferAdd(head: BinaryBuffer, body: BinaryBuffer): boolean {
    ...
    private _bufferTake(): BinaryBuffer {
        const ret = BinaryBuffer.concat(this._data, this._totalLength);
        this._data.length = 0;
        this._totalLength = 0;
        return ret;
    }

    private _writeSoon(header: BinaryBuffer, data: BinaryBuffer): void {
...
export class Protocol extends Disposable implements IMessagePassingProtocol {
    ...
    readonly onMessage: Event<BinaryBuffer> = this._onMessage.event;
...
class LoadEstimator {
    ...
    static getInstance(): LoadEstimator {
...
export interface PersistentProtocolOptions {
     ...
     */
    initialChunk?: BinaryBuffer | null;
    /**
...
export class PersistentProtocol implements IMessagePassingProtocol {
    ...
    private readonly _onControlMessage = new BufferedEmitter<BinaryBuffer>();
    readonly onControlMessage: Event<BinaryBuffer> = this._onControlMessage.event;

    private readonly _onMessage = new BufferedEmitter<BinaryBuffer>();
    readonly onMessage: Event<BinaryBuffer> = this._onMessage.event;
    ...
    beginAcceptReconnection(socket: ISocket, initialDataChunk: BinaryBuffer | null): void {
     ...
     */
    sendControl(buffer: BinaryBuffer): void {

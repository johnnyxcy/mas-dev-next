/*
 * File: ipc/src/common/ipc.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/18/2023 03:13 pm
 *
 * Last Modified: 10/18/2023 03:14 pm
 *
 * Modified By: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * Copyright (c) 2023 Maspectra Dev Team
 */
import { getRandomElement } from "@mas/base/common/arrays";
import { CancelablePromise, createCancelablePromise, timeout } from "@mas/base/common/async";
...
import { CancellationError, ErrorNoTelemetry } from "@mas/base/common/errors";
import { Emitter, Event, EventMultiplexer, Relay } from "@mas/base/common/event";
import { DisposableStore, IDisposable, combinedDisposable, dispose, toDisposable } from "@mas/base/common/lifecycle";
import { revive } from "@mas/base/common/marshalling";
import * as strings from "@mas/base/common/strings";
import { isFunction, isUndefinedOrNull } from "@mas/base/common/types";
...
interface IReader {
    read(bytes: number): BinaryBuffer;
}
...
function writeInt32VQL(writer: IWriter, value: number) {
    ...
    const scratch = BinaryBuffer.alloc(len);
...
export class BufferReader implements IReader {
    ...
    constructor(private buffer: BinaryBuffer) {}
...
export class BufferWriter implements IWriter {
    ...
    get buffer(): BinaryBuffer {
        return BinaryBuffer.concat(this.buffers);
    }
...
function createOneByteBuffer(value: number): BinaryBuffer {
    const result = BinaryBuffer.alloc(1);
...
const BufferPresets = {
    ...
    BinaryBuffer: createOneByteBuffer(DataType.BinaryBuffer),
...
export function serialize(writer: IWriter, data: any): void {
    if (typeof data === "undefined") {
        ...
        const buffer = BinaryBuffer.fromString(data);
        ...
        writeInt32VQL(writer, buffer.byteLength);
        writer.write(buffer);
    } else if (data instanceof BinaryBuffer) {
        writer.write(BufferPresets.BinaryBuffer);
        ...
        const buffer = BinaryBuffer.fromString(JSON.stringify(data));
...
export class ChannelServer<TContext = string> implements IChannelServer<TContext>, IDisposable {
    ...
    private sendBuffer(message: BinaryBuffer): number {
        try {
    ...
    private onRawMessage(message: BinaryBuffer): void {
    ...
    private onEventListen(request: IRawEventListenRequest): void {
        ...
        const disposable = event((data) =>
            this.sendResponse(<IRawResponse>{ id, data, type: ResponseType.EventFire }),
        );
...
export class ChannelClient implements IChannelClient, IDisposable {
    ...
    private onBuffer(message: BinaryBuffer): void {
        const reader = new BufferReader(message);
        const header = deserialize(reader);
        const body = deserialize(reader);
...
function logWithColors(
    ...
    let args = [
        `%c[${direction}]%c[${String(totalLength).padStart(7, " ")}]%c[len: ${String(msgLength).padStart(
            5,
            " ",
        )}]%c${String(req).padStart(5, " ")} - ${str}`,
        "color: darkgreen",
        "color: grey",
        "color: grey",
        `color: ${color}`,

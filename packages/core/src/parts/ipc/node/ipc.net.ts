...
import { Server as NetServer, Socket, createConnection, createServer } from "node:net";
import { tmpdir } from "node:os";
import { DeflateRaw, InflateRaw, ZlibOptions, createDeflateRaw, createInflateRaw } from "node:zlib";
...
import { Disposable, IDisposable } from "@mas/base/common/lifecycle";
import { join } from "@mas/base/common/path";
import { Platform, platform } from "@mas/base/common/platform";
import { generateUuid } from "@mas/base/common/uuid";
import { ClientConnectionEvent, IPCServer } from "@mas/core/parts/ipc/common/ipc";
import {
    ChunkStream,
    Client,
    ISocket,
...
} from "@mas/core/parts/ipc/common/ipc.net";
...
export class NodeSocket implements ISocket {
    ...
    traceSocketEvent(
        ...
        data?: BinaryBuffer | Uint8Array | ArrayBuffer | ArrayBufferView | any,
    ): void {
    ...
    write(buffer: BinaryBuffer): void {
...
export class WebSocketNodeSocket extends Disposable implements ISocket, ISocketTracer {
    ...
    private readonly _onData = this._register(new Emitter<BinaryBuffer>());
    ...
    get recordedInflateBytes(): BinaryBuffer {
    ...
    constructor(
        ...
        inflateBytes: BinaryBuffer | null,
    ...
    onData(listener: (e: BinaryBuffer) => void): IDisposable {
    ...
    private _write(buffer: BinaryBuffer, { compressed, opcode }: FrameOptions): void {
        if (this._isEnded) {
        ...
        const header = BinaryBuffer.alloc(headerLen);
        ...
        this.socket.write(BinaryBuffer.concat([header, buffer]));
    }
    ...
    private _acceptChunk(data: BinaryBuffer): void {
...
class WebSocketFlowManager extends Disposable {
    ...
    private readonly _writeQueue: { data: BinaryBuffer; options: FrameOptions }[] = [];
    private readonly _readQueue: { data: BinaryBuffer; isCompressed: boolean; isLastFrameOfMessage: boolean }[] = [];

    private readonly _onDidFinishProcessingReadQueue = this._register(new Emitter<void>());
    readonly onDidFinishProcessingReadQueue = this._onDidFinishProcessingReadQueue.event;

    private readonly _onDidFinishProcessingWriteQueue = this._register(new Emitter<void>());
    readonly onDidFinishProcessingWriteQueue = this._onDidFinishProcessingWriteQueue.event;
    ...
    constructor(
        ...
        inflateBytes: BinaryBuffer | null,
        recordInflateBytes: boolean,
        private readonly _onData: Emitter<BinaryBuffer>,
        private readonly _writeFn: (data: BinaryBuffer, options: FrameOptions) => void,
    ) {
        super();
    ...
    writeMessage(data: BinaryBuffer, options: FrameOptions): void {
    ...
    isProcessingWriteQueue(): boolean {
     ...
     */
    private _deflateMessage(zlibDeflateStream: ZlibDeflateStream, buffer: BinaryBuffer): Promise<BinaryBuffer> {
        return new Promise<BinaryBuffer>((resolve, reject) => {
            zlibDeflateStream.write(buffer);
            zlibDeflateStream.flush((data) => resolve(data));
        });
    }

    acceptFrame(data: BinaryBuffer, isCompressed: boolean, isLastFrameOfMessage: boolean): void {
    ...
    private _inflateFrame(
        ...
        return new Promise<BinaryBuffer>((resolve, reject) => {
            ...
            if (isLastFrameOfMessage) {
                zlibInflateStream.write(BinaryBuffer.fromByteArray([0x00, 0x00, 0xff, 0xff]));
            }
...
class ZlibInflateStream extends Disposable {
    ...
    private readonly _recordedInflateBytes: BinaryBuffer[] = [];
    private readonly _pendingInflateData: BinaryBuffer[] = [];

    get recordedInflateBytes(): BinaryBuffer {
        if (this._recordInflateBytes) {
            return BinaryBuffer.concat(this._recordedInflateBytes);
        }
        return BinaryBuffer.alloc(0);
    }
    ...
    constructor(
        ...
        this._zlibInflate.on("data", (data: Buffer) => {
            ...
            this._pendingInflateData.push(BinaryBuffer.wrap(data));
        });
        if (inflateBytes) {
    ...
    flush(callback: (data: BinaryBuffer) => void): void {
        this._zlibInflate.flush(() => {
            this._tracer.traceSocketEvent(SocketDiagnosticsEventType.zlibInflateFlushFired);
            const data = BinaryBuffer.concat(this._pendingInflateData);
            this._pendingInflateData.length = 0;
            callback(data);
        });
    }
...
class ZlibDeflateStream extends Disposable {
    ...
    private readonly _pendingDeflateData: BinaryBuffer[] = [];
    ...
    constructor(
        ...
        this._zlibDeflate.on("data", (data: Buffer) => {
            ...
            this._pendingDeflateData.push(BinaryBuffer.wrap(data));
        });
    }
    ...
    flush(callback: (data: BinaryBuffer) => void): void {
        ...
        this._zlibDeflate.flush(/* Z_SYNC_FLUSH */ 2, () => {
            ...
            let data = BinaryBuffer.concat(this._pendingDeflateData);
...
function unmask(buffer: BinaryBuffer, mask: number): void {
    if (mask === 0) {
        return;
    }
...
const safeIpcPathLengths: { readonly [platform: number]: number } = {
    [Platform.Linux]: 107,
    [Platform.Mac]: 103,
};

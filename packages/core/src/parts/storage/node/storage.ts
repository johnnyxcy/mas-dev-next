/*
 * File: @mas/core/src/parts/storage/node/storage.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/25/2023 02:00 pm
 *
 * Last Modified: 10/25/2023 02:04 pm
 *
...
import { mapToString, setToString } from "@mas/base/common/map";
import { basename } from "@mas/base/common/path";
import { Promises } from "@mas/base/node/pfs";
import { IStorageDatabase, IStorageItemsChangeEvent, IUpdateRequest } from "@mas/core/parts/storage/common/storage";
...
export class SQLiteStorageDatabase implements IStorageDatabase {
    ...
    constructor(
        ...
        private readonly options: ISQLiteStorageDatabaseOptions = Object.create(null),
    ) {}
    ...
    private doUpdateItems(connection: IDatabaseConnection, request: IUpdateRequest): Promise<void> {
        if (this.logger.isTracing) {
            this.logger.trace(
                `[storage ${this.name}] updateItems(): insert(${
                    request.insert ? mapToString(request.insert) : "0"
                }), delete(${request.delete ? setToString(request.delete) : "0"})`,
            );
        }
        ...
        return this.transaction(connection, () => {
            ...
            if (toInsert && toInsert.size > 0) {
                ...
                keysValuesChunks.forEach((keysValuesChunk) => {
                    this.prepare(
                        connection,
                        `INSERT INTO ItemTable VALUES ${new Array(keysValuesChunk.length / 2)
                            .fill("(?,?)")
                            .join(",")}`,
        ...
        return new Promise((resolve, reject) => {
            import("@vscode/sqlite3").then((sqlite3) => {
                const connection: IDatabaseConnection = {
                    db: new (this.logger.isTracing ? sqlite3.verbose().Database : sqlite3.Database)(
                        path,

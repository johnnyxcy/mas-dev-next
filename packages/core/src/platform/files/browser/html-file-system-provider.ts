/*
 * File: @mas/core/src/platform/files/browser/html-file-system-provider.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/02/2023 04:10 pm
 *
 * Last Modified: 11/02/2023 04:11 pm
 *
...
import { basename, extname, normalize } from "@mas/base/common/path";
import { isLinux } from "@mas/base/common/platform";
import { extUri, extUriIgnorePathCase } from "@mas/base/common/resources";
import { ReadableStreamEvents, newWriteableStream } from "@mas/base/common/stream";
import { URI } from "@mas/base/common/uri";
import { WebFileSystemAccess } from "@mas/core/platform/files/browser/web-file-system-access";
import {
    ...
    implements IFileSystemProviderWithFileReadWriteCapability, IFileSystemProviderWithFileReadStreamCapability
{
    ...
    get capabilities(): FileSystemProviderCapabilities {
        if (!this._capabilities) {
            this._capabilities =
                FileSystemProviderCapabilities.FileReadWrite | FileSystemProviderCapabilities.FileReadStream;
    ...
    async delete(resource: URI, opts: IFileDeleteOptions): Promise<void> {
        try {
            ...
            return await parent.removeEntry(this.extUri.basename(resource), { recursive: opts.recursive });
        } catch (error) {
            throw this.toFileSystemProviderError(error);
        }
    }
    ...
    async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<void> {
        try {
            ...
            // File API does not support any real rename otherwise
                ...
                throw this.createFileSystemProviderError(
                    from,
                    nls.localizeByDefault("Rename is only supported for files."),
    ...
    private async registerHandle(handle: FileSystemHandle, map: Map<string, FileSystemHandle>): Promise<URI> {
        ...
        try {
            await this.indexedDB?.runInTransaction(this.store, "readwrite", (objectStore) =>
                objectStore.put(handle, handleId),
            );
        } catch (error) {
    ...
    private toFileSystemProviderError(error: Error): FileSystemProviderError {
        ...
        if (error.name === "NotAllowedError") {
            error = new Error(
                nls.localizeByDefault("Insufficient permissions. Please retry and allow the operation."),
            );

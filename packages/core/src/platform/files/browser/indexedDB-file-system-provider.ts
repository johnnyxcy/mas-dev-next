...
import { BroadcastDataChannel } from "@mas/base/browser/broadcast";
import { DBClosedError, IndexedDB } from "@mas/base/browser/indexedDB";
import { Throttler } from "@mas/base/common/async";
...
import { ExtUri } from "@mas/base/common/resources";
import { isString } from "@mas/base/common/types";
import { URI, UriDto } from "@mas/base/common/uri";
import {
    FileChangeType,
...
const ERR_UNKNOWN_INTERNAL = (message: string) =>
    createFileSystemProviderError(
        nls.localize("internal", "Internal error occurred in IndexedDB File System Provider. ({0})", message),
        FileSystemProviderErrorCode.Unknown,
    );
...
export class IndexedDBFileSystemProvider extends Disposable implements IFileSystemProviderWithFileReadWriteCapability {
    ...
    constructor(
        ...
        if (watchCrossWindowChanges) {
            this.changesBroadcastChannel = this._register(
                new BroadcastDataChannel<UriDto<IFileChange>[]>(`vscode.indexedDB.${scheme}.changes`),
            );
            this._register(
                this.changesBroadcastChannel.onDidReceiveData((changes) => {
                    this._onDidChangeFile.fire(
                        changes.map((c) => ({ type: c.type, resource: URI.revive(c.resource) })),
                    );
                }),
            );
        }
    ...
    async readFile(resource: URI): Promise<Uint8Array> {
        try {
            const result = await this.indexedDB.runInTransaction(this.store, "readonly", (objectStore) =>
                objectStore.get(resource.path),
            );
            if (result === undefined) {
                throw ERR_FILE_NOT_FOUND;
            }
            const buffer =
                result instanceof Uint8Array
                    ? result
                    : isString(result)
                    ? BinaryBuffer.fromString(result).buffer
                    : undefined;
            if (buffer === undefined) {
    ...
    async rename(from: URI, to: URI, opts: IFileOverwriteOptions): Promise<void> {
        ...
        const toTargetResource = (path: string): URI =>
            this.extUri.joinPath(to, this.extUri.relativePath(from, from.with({ path })) || "");
        ...
        if (sourceFiles.length) {
            ...
            const sourceFilesContents = await this.indexedDB.runInTransaction(this.store, "readonly", (objectStore) =>
            ...
            for (let index = 0; index < sourceFiles.length; index++) {
                const content =
                    sourceFilesContents[index] instanceof Uint8Array
                        ...
                        ? BinaryBuffer.fromString(sourceFilesContents[index]).buffer
                        : undefined;
                if (content) {
    ...
    async delete(resource: URI, opts: IFileDeleteOptions): Promise<void> {
        ...
        this.triggerChanges(
            toDelete.map((path) => ({ resource: resource.with({ path }), type: FileChangeType.DELETED })),
        );
    }
    ...
    private async deleteKeys(keys: string[]): Promise<void> {
        if (keys.length) {
            await this.indexedDB.runInTransaction(this.store, "readwrite", (objectStore) =>
                keys.map((key) => objectStore.delete(key)),
            );
        }
    }

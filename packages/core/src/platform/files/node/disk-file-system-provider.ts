/*
 * File: @mas/core/src/platform/files/node/disk-file-system-provider.ts
 *
...
import { Barrier, retry } from "@mas/base/common/async";
...
import { isEqual } from "@mas/base/common/extpath";
import { DisposableStore, IDisposable, toDisposable } from "@mas/base/common/lifecycle";
import { ResourceMap } from "@mas/base/common/map";
import { basename, dirname, join } from "@mas/base/common/path";
...
} from "@mas/base/common/resources";
import { ReadableStreamEvents, newWriteableStream } from "@mas/base/common/stream";
import { URI } from "@mas/base/common/uri";
import { IDirent, Promises, RimRafMode, SymlinkSupport } from "@mas/base/node/pfs";
import {
    AbstractDiskFileSystemProvider,
    IDiskFileSystemProviderOptions,
} from "@mas/core/platform/files/common/disk-file-system-provider";
import {
    FilePermission,
...
import { UniversalWatcherClient } from "@mas/core/platform/files/node/watcher/client";
import { NodeJSWatcherClient } from "@mas/core/platform/files/node/watcher/nodejs/client";
        ...
        IFileSystemProviderWithFileCloneCapability
{
    ...
    readFileStream(
        ...
        const stream = newWriteableStream<Uint8Array>(
            (data) => BinaryBuffer.concat(data.map((data) => BinaryBuffer.wrap(data))).buffer,
        );
    ...
    async writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<void> {
        if (opts?.atomic !== false && opts?.atomic?.postfix && (await this.canWriteFileAtomic(resource))) {
            return this.doWriteFileAtomic(
                resource,
                joinPath(resourcesDirname(resource), `${resourcesBasename(resource)}${opts.atomic.postfix}`),
                content,
                opts,
            );
        } else {
                ...
                if (fileExists) {
                    if (!opts.overwrite) {
                        throw createFileSystemProviderError(
                            nls.localizeByDefault("File already exists"),
                    ...
                    if (!opts.create) {
                        throw createFileSystemProviderError(
                            nls.localizeByDefault("File does not exist"),
    ...
    private async validateMoveCopy(from: URI, to: URI, mode: "move" | "copy", overwrite?: boolean): Promise<void> {
        ...
        if (isSameResourceWithDifferentPathCase) {
            ...
            if (mode === "copy") {
                throw createFileSystemProviderError(
                    nls.localizeByDefault("File cannot be copied to same path with different path case"),
                    FileSystemProviderErrorCode.FileExists,
                );
            }
        ...
        if (!fromStat) {
            throw createFileSystemProviderError(
                nls.localizeByDefault("File to move/copy does not exist"),
                FileSystemProviderErrorCode.FileNotFound,
            );
        }

/*
 * File: @mas/core/src/platform/files/node/disk-file-system-provider-service.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/06/2023 02:01 pm
 *
 * Last Modified: 11/06/2023 04:02 pm
 *
...
import { CancellationTokenSource } from "@mas/base/common/cancellation";
import { Emitter, Event } from "@mas/base/common/event";
import { Disposable, IDisposable, dispose, toDisposable } from "@mas/base/common/lifecycle";
import { ReadableStreamEventPayload, listenStream } from "@mas/base/common/stream";
import { URI, UriComponents } from "@mas/base/common/uri";
import { IURITransformer } from "@mas/base/common/uri-ipc";
import { IServerChannel } from "@mas/core/parts/ipc/common/ipc";
...
import { IRecursiveWatcherOptions } from "@mas/core/platform/files/common/watcher";
...
export abstract class AbstractDiskFileSystemProviderChannel<T> extends Disposable implements IServerChannel<T> {
    ...
    private onReadFileStream(
    ...
    ): Event<ReadableStreamEventPayload<BinaryBuffer>> {
        const resource = this.transformIncoming(uriTransformer, _resource, true);
        const cts = new CancellationTokenSource();

        const emitter = new Emitter<ReadableStreamEventPayload<BinaryBuffer>>({
        ...
        listenStream(fileStream, {
            onData: (chunk) => emitter.fire(BinaryBuffer.wrap(chunk)),
    ...
    private async read(fd: number, pos: number, length: number): Promise<[BinaryBuffer, number]> {
        const buffer = BinaryBuffer.alloc(length);
    ...
    private write(fd: number, pos: number, data: BinaryBuffer, offset: number, length: number): Promise<number> {
...
export abstract class AbstractSessionFileWatcher extends Disposable implements ISessionFileWatcher {
    ...
    private readonly fileWatcher = this._register(
        new DiskFileSystemProvider(this.logService, {
            watcher: { recursive: this.getRecursiveWatcherOptions(this.environmentService) },
        }),
    );

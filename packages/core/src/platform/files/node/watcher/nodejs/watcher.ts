...
import { patternsEquals } from "@mas/base/common/glob";
...
import { ILogMessage, INonRecursiveWatchRequest, INonRecursiveWatcher } from "@mas/core/platform/files/common/watcher";
...
export class NodeJSWatcher extends Disposable implements INonRecursiveWatcher {
    ...
    async watch(requests: INonRecursiveWatchRequest[]): Promise<void> {
        ...
        const pathsToStopWatching = Array.from(this.watchers.values())
            .filter(({ request }) => {
                return !normalizedRequests.some(
                    (normalizedRequest) =>

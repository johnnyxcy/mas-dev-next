...
import { RunOnceWorker, ThrottledWorker } from "@mas/base/common/async";
import { CancellationToken, CancellationTokenSource } from "@mas/base/common/cancellation";
import { isEqualOrParent } from "@mas/base/common/extpath";
...
import { isLinux, isMacintosh } from "@mas/base/common/platform";
import { joinPath } from "@mas/base/common/resources";
import { URI } from "@mas/base/common/uri";
import { realcase } from "@mas/base/node/extpath";
...
export class NodeJSFileWatcherLibrary extends Disposable {
    ...
    private async existsChildStrictCase(path: string): Promise<boolean> {
        ...
        try {
            ...
            return children.includes(pathBasename);
        } catch (error) {
            this.trace(error);

/*
 * File: @mas/core/src/platform/files/common/files.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/02/2023 02:37 pm
 *
 * Last Modified: 11/02/2023 02:55 pm
 *
...
import { BinaryBuffer, BinaryBufferReadable, BinaryBufferReadableStream } from "@mas/base/common/buffer";
import { CancellationToken } from "@mas/base/common/cancellation";
import { Event } from "@mas/base/common/event";
import { IExpression, IRelativePattern } from "@mas/base/common/glob";
import { IMarkdownString } from "@mas/base/common/html-content";
...
import { sep } from "@mas/base/common/path";
import { isWeb } from "@mas/base/common/platform";
import { ReadableStreamEvents } from "@mas/base/common/stream";
import { startsWithIgnoreCase } from "@mas/base/common/strings";
import { TernarySearchTree } from "@mas/base/common/ternary-search-tree";
import { isNumber } from "@mas/base/common/types";
import { URI } from "@mas/base/common/uri";

// #region file service & providers

export const IFileService = Symbol("fileService");
...
export function ensureFileSystemProviderError(error?: Error): Error {
    if (!error) {
        return createFileSystemProviderError(
            nls.localizeByDefault("Unknown Error"),
...
export interface IFileStreamContent extends IBaseFileStatWithMetadata {
     ...
     */
    readonly value: BinaryBufferReadableStream;
}
...
export class ByteSize {
    ...
    static formatSize(size: number): string {
        ...
        if (size < ByteSize.KB) {
            return nls.localizeByDefault("{0}B", size.toFixed(0));
        }

        if (size < ByteSize.MB) {
            return nls.localizeByDefault("{0}KB", (size / ByteSize.KB).toFixed(2));
        }

        if (size < ByteSize.GB) {
            return nls.localizeByDefault("{0}MB", (size / ByteSize.MB).toFixed(2));
        }

        if (size < ByteSize.TB) {
            return nls.localizeByDefault("{0}GB", (size / ByteSize.GB).toFixed(2));
        }

        return nls.localizeByDefault("{0}TB", (size / ByteSize.TB).toFixed(2));
    }
}

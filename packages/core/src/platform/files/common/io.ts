/*
 * File: @mas/core/src/platform/files/common/io.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 11/02/2023 03:02 pm
 *
 * Last Modified: 11/02/2023 03:03 pm
 *
...
import { IDataTransformer, IErrorTransformer, WriteableStream } from "@mas/base/common/stream";
...
export async function readFileIntoStream<T>(
    ...
    transformer: IDataTransformer<BinaryBuffer, T>,
...
function throwIfTooLarge(totalBytesRead: number, options: ICreateReadStreamOptions): boolean {
    ...
    if (typeof options?.limits?.size === "number" && totalBytesRead > options.limits.size) {
        throw createFileSystemProviderError(
            nls.localizeByDefault("File is too large to open"),

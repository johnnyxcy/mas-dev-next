...
import { coalesce } from "@mas/base/common/arrays";
import { Promises, ResourceQueue } from "@mas/base/common/async";
import {
    BinaryBuffer,
...
import { inject } from "@mas/base/common/di";
import { ErrorNoTelemetry } from "@mas/base/common/errors";
import { Emitter } from "@mas/base/common/event";
import { hash } from "@mas/base/common/hash";
import { Iterable } from "@mas/base/common/iterator";
import { Disposable, DisposableStore, IDisposable, dispose, toDisposable } from "@mas/base/common/lifecycle";
import { Schemas } from "@mas/base/common/network";
import { mark } from "@mas/base/common/performance";
import { IExtUri, extUri, extUriIgnorePathCase, isAbsolutePath } from "@mas/base/common/resources";
import {
    consumeStream,
    ...
    peekStream,
    transform,
} from "@mas/base/common/stream";
import { TernarySearchTree } from "@mas/base/common/ternary-search-tree";
...
import { readFileIntoStream } from "@mas/core/platform/files/common/io";
import { ILogService } from "@mas/core/platform/log/common/log";
...
export class FileService extends Disposable implements IFileService {
    ...
    constructor(@inject(ILogService) private readonly logService: ILogService) {
        super();
    }
    ...
    protected async withProvider(resource: URI): Promise<IFileSystemProvider> {
        ...
        if (!provider) {
            ...
            error.message = nls.localizeByDefault(
    ...
    private async withWriteProvider(
        resource: URI,
    ): Promise<IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithOpenReadWriteCloseCapability> {
        const provider = await this.withProvider(resource);
    ...
    async resolve(resource: URI, options?: IResolveFileOptions): Promise<IFileStat> {
        try {
            ...
            if (toFileSystemProviderErrorCode(error) === FileSystemProviderErrorCode.FileNotFound) {
                throw new FileOperationError(
                    nls.localizeByDefault("Unable to resolve nonexistent file '{0}'", this.resourceForError(resource)),
    ...
    private async toFileStat(
        ...
        const fileStat: IFileStat = {
            ...
            readonly:
                ...
                Boolean(provider.capabilities & FileSystemProviderCapabilities.Readonly),
    ...
    async createFile(
        ...
        bufferOrReadableOrStream:
            | BinaryBuffer
            | BinaryBufferReadable
            | BinaryBufferReadableStream = BinaryBuffer.fromString(""),
    ...
    async writeFile(
        resource: URI,
        bufferOrReadableOrStream: BinaryBuffer | BinaryBufferReadable | BinaryBufferReadableStream,
        ...
        try {
            ...
            if (
                ...
                (hasReadWriteCapability(provider) &&
                    bufferOrReadableOrStreamOrBufferedStream instanceof BinaryBuffer) || // data is a full buffer already
                (hasReadWriteCapability(provider) &&
            ...
            // write file: buffered
                ...
                await this.doWriteBuffered(
                    ...
                    resource,
                    writeFileOptions,
                    bufferOrReadableOrStreamOrBufferedStream instanceof BinaryBuffer
    ...
    private async validateWriteFile(
        ...
        if (
            ...
            options.etag !==
                etag({ mtime: options.mtime /* not using stat.mtime for a reason, see above */, size: stat.size })
        ) {
            throw new FileOperationError(
                nls.localizeByDefault("File Modified Since"),
                FileOperationResult.FILE_MODIFIED_SINCE,
                options,
            );
        }
    ...
    private async doReadFileStream(
        provider:
            ...
            | IFileSystemProviderWithFileReadStreamCapability,
        resource: URI,
        options?: IReadFileOptions & IReadFileStreamOptions & { preferUnbuffered?: boolean },
        token?: CancellationToken,
    ): Promise<IFileStreamContent> {
        ...
        const statPromise = this.validateReadFile(resource, readFileOptions).then(
            (stat) => stat,
            (error) => {
        ...
        let fileStream: BinaryBufferReadableStream | undefined = undefined;
        try {
    ...
    private restoreReadError(error: Error, resource: URI, options?: IReadFileStreamOptions): FileOperationError {
        const message = nls.localizeByDefault(
    ...
    private readFileUnbuffered(
        provider: IFileSystemProviderWithFileReadWriteCapability | IFileSystemProviderWithFileAtomicReadCapability,
        resource: URI,
        options?: IReadFileOptions & IReadFileStreamOptions,
    ): BinaryBufferReadableStream {
        const stream = newWriteableStream<BinaryBuffer>((data) => BinaryBuffer.concat(data));
        ...
        (async () => {
            try {
                ...
                stream.end(BinaryBuffer.wrap(buffer));
            } catch (err) {
                stream.error(err);
                stream.end();
            }
    ...
    private async validateReadFile(resource: URI, options?: IReadFileStreamOptions): Promise<IFileStatWithMetadata> {
        ...
        if (typeof options?.etag === "string" && options.etag !== ETAG_DISABLED && options.etag === stat.etag) {
            throw new NotModifiedSinceFileOperationError(
                nls.localizeByDefault("File not modified since"),
                stat,
                options,
            );
        }
    ...
    async move(source: URI, target: URI, overwrite?: boolean): Promise<IFileStatWithMetadata> {
        ...
        this._onDidRunOperation.fire(
            new FileOperationEvent(source, mode === "move" ? FileOperation.MOVE : FileOperation.COPY, fileStat),
        );
    ...
    async copy(source: URI, target: URI, overwrite?: boolean): Promise<IFileStatWithMetadata> {
        ...
        const fileStat = await this.resolve(target, { resolveMetadata: true });
        this._onDidRunOperation.fire(
            new FileOperationEvent(source, mode === "copy" ? FileOperation.COPY : FileOperation.MOVE, fileStat),
        );
        ...
        if (Array.isArray(sourceFolder.children)) {
            await Promises.settled(
                sourceFolder.children.map(async (sourceChild) => {
                    const targetChild = this.getExtUri(targetProvider).providerExtUri.joinPath(
                        targetFolder,
                        sourceChild.name,
                    );
    ...
    private async doValidateDelete(
        ...
        if (stat) {
            ...
            throw new FileOperationError(
                nls.localizeByDefault("Unable to delete nonexistent file '{0}'", this.resourceForError(resource)),
                FileOperationResult.FILE_NOT_FOUND,
            );
        }
        ...
        if (!recursive) {
            ...
            if (stat.isDirectory && Array.isArray(stat.children) && stat.children.length > 0) {
                throw new Error(
                    nls.localizeByDefault("Unable to delete non-empty folder '{0}'.", this.resourceForError(resource)),
                );
            }
        }
    ...
    private async doWriteStreamBufferedQueued(
        provider: IFileSystemProviderWithOpenReadWriteCloseCapability,
        handle: number,
        streamOrBufferedStream: BinaryBufferReadableStream | BinaryBufferReadableBufferedStream,
    ): Promise<void> {
        let posInFile = 0;
    ...
    private async doWriteUnbuffered(
        ...
        return this.writeQueue
            .queueFor(resource, this.getExtUri(provider).providerExtUri)
            .queue(() =>
                this.doWriteUnbufferedQueued(provider, resource, options, bufferOrReadableOrStreamOrBufferedStream),
            );
    }
    ...
    private async doPipeBuffered(
        ...
        return this.writeQueue
            .queueFor(target, this.getExtUri(targetProvider).providerExtUri)
            .queue(() => this.doPipeBufferedQueued(sourceProvider, source, targetProvider, target));
    }
    ...
    private async doPipeUnbuffered(
        ...
        return this.writeQueue
            .queueFor(target, this.getExtUri(targetProvider).providerExtUri)
            .queue(() => this.doPipeUnbufferedQueued(sourceProvider, source, targetProvider, target));
    }
    ...
    private async doPipeUnbufferedToBuffered(
        ...
        return this.writeQueue
            .queueFor(target, this.getExtUri(targetProvider).providerExtUri)
            .queue(() => this.doPipeUnbufferedToBufferedQueued(sourceProvider, source, targetProvider, target));
    }
    ...
    protected throwIfFileSystemIsReadonly<T extends IFileSystemProvider>(provider: T, resource: URI): T {
        if (provider.capabilities & FileSystemProviderCapabilities.Readonly) {
            throw new FileOperationError(
                nls.localizeByDefault("Unable to modify read-only file '{0}'", this.resourceForError(resource)),

...
import { ReadableStreamEventPayload, ReadableStreamEvents, newWriteableStream } from "@mas/base/common/stream";
import { URI } from "@mas/base/common/uri";
import { generateUuid } from "@mas/base/common/uuid";
import { IChannel } from "@mas/core/parts/ipc/common/ipc";
import {
...
import { reviveFileChanges } from "@mas/core/platform/files/common/watcher";
        ...
        IFileSystemProviderWithFileCloneCapability
{
    ...
    async readFile(resource: URI, opts?: IFileAtomicReadOptions): Promise<Uint8Array> {
        const { buffer } = (await this.channel.call("readFile", [resource, opts])) as BinaryBuffer;
    ...
    readFileStream(
        ...
        disposables.add(
            this.channel.listen<ReadableStreamEventPayload<BinaryBuffer>>("readFileStream", [resource, opts])(
                (dataOrErrorOrEnd) => {
                    // data
                    if (dataOrErrorOrEnd instanceof BinaryBuffer) {
    ...
    writeFile(resource: URI, content: Uint8Array, opts: IFileWriteOptions): Promise<void> {
        return this.channel.call("writeFile", [resource, BinaryBuffer.wrap(content), opts]);
    }
    ...
    async read(fd: number, pos: number, data: Uint8Array, offset: number, length: number): Promise<number> {
        const [bytes, bytesRead]: [BinaryBuffer, number] = await this.channel.call("read", [fd, pos, length]);
    ...
    write(fd: number, pos: number, data: Uint8Array, offset: number, length: number): Promise<number> {
        return this.channel.call("write", [fd, pos, BinaryBuffer.wrap(data), offset, length]);
    }

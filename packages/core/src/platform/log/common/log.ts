/*
 * File: @mas/core/src/platform/log/common/log.ts
 *
 * Author: Johnny Xu <johnny.xcy1997@outlook.com>
 *
 * File Created: 10/18/2023 01:24 pm
 *
 * Last Modified: 10/18/2023 01:47 pm
 *
...
import { Mutable, isNumber, isString } from "@mas/base/common/types";
...
export class AdapterLogger extends AbstractLogger implements ILogger {
    constructor(
        private readonly adapter: { log: (logLevel: LogLevel, args: any[]) => void },
        logLevel: LogLevel = DEFAULT_LOG_LEVEL,
    ) {
        super();
        this.setLevel(logLevel);
...
export function getLogLevel(configuration: { verbose?: boolean; logLevel?: string }): LogLevel {
    if (configuration.verbose) {
        return LogLevel.Trace;
    }
    if (typeof configuration.logLevel === "string") {
        const logLevel = parseLogLevel(configuration.logLevel.toLowerCase());

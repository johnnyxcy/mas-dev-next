...
def evaluate_expression(valobj, expr):
    return valobj.GetProcess().GetSelectedThread().GetSelectedFrame(
...
class Matrix(Printer):
    ...
    def __init__(self, variety, val):
        try:
            ...
            if self.rows == -1:
                self.rows = val.GetValueForExpressionPath(
                    ".m_storage.m_rows").GetValueAsSigned()

            self.cols = int(template_params[2])
            if self.cols == -1:
                self.cols = val.GetValueForExpressionPath(
                    ".m_storage.m_cols").GetValueAsSigned()
...
class SparseMatrix(Printer):
        ...
        try:
            ...
            if (innerNNZs.GetValueAsSigned()):
                for k in range(0, self.rows if self.rowMajor else self.cols):
                    self.innerNNZs.append(
                        evaluate_at_index(innerNNZs, k).GetValueAsSigned())
            else:
                for k in range(0, self.rows if self.rowMajor else self.cols):
                    self.innerNNZs.append(
                        evaluate_at_index(self.outerStarts, k +
                                          1).GetValueAsSigned() -
    ...
    def to_string(self):
        ...
        output = "rows: %d, cols: %d, nonzeros: %d\n{ " % (
            self.rows, self.cols, self.nonzeros)
...
class Quaternion(Printer):
    ...
    def __init__(self, val):
        try:
            ...
            data = val.GetValueForExpressionPath(
